...
module.exports = (db) => {
  ...
  // add a poll
  router.post("/", (req, res) => {
    let poll_question = req.body.poll_question;
    let user_email = req.body.user_email;
    let choices = req.body.choices;
    //generate guid code re
    //var guid = '555';
    let guid = getRandomInt(10000, 99999);
    //generate links
    let submission_link = 'http://localhost:8080/' + 'submission_page' +  '?pollCode=' + guid;
    let admin_link = 'http://localhost:8080/' + 'admin_page' + '?pollCode=' + guid;
    const msg = {
